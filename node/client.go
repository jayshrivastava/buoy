...
func (client *client) requestVotes(term int32, raftNodeId int32, lastLogIndex int32, lastLogTerm int32) (REQUEST_VOTES_RETURN_TYPE, int32) {
	votesReceived := 1
	maxTermSeen := term
	// use go routines here to send the requests in parallel
	responses := []*rpc.RequestVotesResponse{}
	mu := sync.Mutex{}
	wg := sync.WaitGroup{}

	for _, nodeClient := range client.rpcClients {
		req := rpc.RequestVotesRequest{
			Term:         term,
		...
		go func(nodeClient rpc.RaftClient) {
			defer wg.Done()
