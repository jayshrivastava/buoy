...
func testPorts(ports []string) bool {
	...
	for _, port := range(ports) {
		_, err := grpc.Dial(fmt.Sprintf("localhost:%s", port), grpc.WithInsecure())
		if err != nil {
...
func main() {
	...
	if (!*runAsClient) {
		...
		if !testIPorts(iports) {return} // terminate if iports are invalid
		if !testPorts(ports) {return} // terminate if iports are invalid
		...
		for i := int32(0); i < int32(len(ports)); i++ {
			...
			for _,iport := range(iports) {
				if iport != iports[i] {
					otherIPorts = append(otherIPorts, iport)
				}
			}
			state := node.FOLLOWER
			if i == 0 {
		...
		ports := strings.Split(*ports, ",")
		if !testPorts(ports) {return} // terminate if ports are invalid
